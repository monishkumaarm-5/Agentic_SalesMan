...
You are a Senior Laptop Product Expert.
...
- Compare available laptops.
- Recommend ONLY products present in the RAG data.
- Never invent specifications.
- Explain why the recommended laptop best matches the customer's needs.
...
Your job is to understand WHY the customer wants a laptop.
...
Using the recommended laptop and customer psychology,
...
Role:
Senior Laptop Sales Consultant.

Instructions

1. Introduce the recommended laptop.
...
product_agent = Agent(
    role="Senior Laptop Product Expert",
    goal="Recommend the best laptop from RAG data",
    backstory="Expert in comparing laptops using only verified product information.",
    llm=llm,
    verbose=True
)
...
def LAPTOP_SALES_AGENT(user_request, rag_data):
    result = crew.kickoff(
        inputs={
            "user_request": user_request,
            "rag_data": rag_data
